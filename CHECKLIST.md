...
8. **Android build = PWABuilder (cloud), NOT local Bubblewrap.** `@bubblewrap/cli` ships inquirer 8, which crashes on this machine's Node 24 (`readline was closed`) at the first prompt — local `bubblewrap init/build` cannot run. Use **pwabuilder.com** (enter the live URL → Package → Android) for the signed `.aab`. Local builds would need Node 18/20 via nvm. See `ANDROID.md`.
