...
7. **Flag emoji are intentionally per-device.** A small script in `PWA_META` detects whether the device renders flag emoji; if not (Windows desktop), it strips the broken two-letter codes, leaving the country name beside them. Phones/Mac/the app show real flags. So "flags missing on desktop" is BY DESIGN — don't "fix" it. The 🌍 globe (unknown-country fallback) and 🌴 (Caribbean) are kept (they render everywhere).
8. **Android app is BUILT (v2: signed, no billing/geolocation, version code 3)** at `D:\bwn-android\app-release-bundle.aab`. For future versions: double-click **`D:\bwn-android\rebuild.bat`** (enter keystore password when asked). The 5 fixes that make local Bubblewrap work on this machine (full recipe in `ANDROID.md`): **Node 20 not 24** (inquirer crash), **CRLF .bat line-endings**, clear **`NoDefaultCurrentDirectoryInExePath`**, **`-Xmx512m`** in gradle.properties, **JDK bin on PATH** (for jarsigner). Keystore: `D:\bwn-android\android.keystore` (outside this public repo — never commit it). PWABuilder.com is the cloud fallback.
...
- [x] Windows Task Scheduler — **agent (`run_all.bat`) every 2 days at 3am** (`BlackWorldNews_Update`) + **daily featured shuffle (`shuffle_daily.bat`) at 5am** (`BlackWorldNews_Shuffle`, no AI/no cost). Both run as glenn/interactive, StartWhenAvailable on (runs when PC next wakes if off at the scheduled time).
...
## 🆕 DONE 2026-05-30
- [x] **Featured picker tuned for editorial substance** — `pick_featured.py` scoring now weights structural depth (colonial legacy, corporate extraction, land theft, foreign debt, engineered unemployment), agency/exploitation framing, and a named beneficiary (`cui_bono`), alongside the big-publication preference. Surfaces substantive stories over fluff; published copy stays neutral wire-service tone. One-click Android build helper at `C:\Users\glenn\build-android-bwn.bat` (Bubblewrap is interactive — must run in a real terminal; cannot build from the agent shell).
- [x] **Daily featured rotation + new cadence** — `pick_featured.py` now auto-picks (no prompt): scores recent stories, strongly prefers big publications (BBC/Reuters/Guardian/Al Jazeera/Bloomberg/NYT/The Conversation/AfricaNews/etc.), excludes Wikipedia/LinkedIn/YouTube/stat-sites/think-tank PDFs, caps 2 per domain, and rotates a fresh hero **every calendar day** (`--choose` still does manual). Agent runs **every 2 days** (`run_all.bat`); a no-cost **daily shuffle** (`shuffle_daily.bat`) rotates the featured + rebuilds + pushes. Cost of daily-agent was the question — answer: only DeepSeek is paid (~$0.0005/story), so even daily would be ~$1–3/mo; running every 2 days + daily free shuffle keeps it fresh for pennies.
- [x] **Comics door live + character bible** — TikTok/Facebook/YouTube channels wired into the footer bar and kids Watch & Learn door (all 3 live). New `comics.html` (kids-styled, no social bar, "first strip coming soon", cast preview) — `COMICS_URL` now points to it, so **both kids doors are live (no more "Coming soon")**. `CHARACTER_BIBLE.md` has paste-ready Dashtoon lock-phrases for Garvey/Toussaint/Yaa + two original kid guides (Kojo & Ama) + a 6-panel first-comic test. Tooling: Dashtoon (free ~100 img/day, character library) recommended; reference-sheet method is the key to consistency. Pollinations can't do consistent characters.
- [x] **Social channel machine + privacy page** — footer social bar on all 5 public pages (YouTube/TikTok/Instagram/Facebook/X/WhatsApp), single source of truth at top of `generate_site.py`, shows only channels with a URL set (kids page excluded by design). New `privacy.html` (required for the Play listing) linked from every public footer; contact email `hello@blackworldnews.world` — change in `build_privacy()` if needed. Sign-up handles + which constant each fills are in the session notes; paste channel URLs and they wire up site-wide.
